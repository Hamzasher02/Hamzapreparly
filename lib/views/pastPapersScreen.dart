...
import '../controllers/pastPapersListController.dart';
...
class _PastPapersScreenState extends State<PastPapersScreen> {
  ...
  final papersListController = Get.put(PastPapersListController());
  ...
  Future<void> _refreshData() async {
    await papersListController.fetchPastPapers();

    if (!papersListController.isLoading.value) {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredList = papersListController.pastPapers
                    ...
                    child: Column(
                      children: [
                        ...
                        Expanded(
                          child: Obx(
                                () {
                                  if (papersListController.pastPapers.isEmpty) {
                                    return Center(
                                      child: PastPapersShimmerItem(),
                                    );
                                  } else if (filteredList.isEmpty) {
                                    return Center(
                                      child: Text("Past Papers will be added soon"),
                                    );
                                  }
                                  else {
                                return ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return InkWell(
                                        onTap: () {
                                          String pastPaperClicked = item['_id']; // sending this 'id' of pastPaper item is also very important so that the filterQuestionsByChapterId () in quiz screen can do comparison and filteration properly using name
                                          String pastPaperItemName = item['name'];

                                          String source = 'pastPaper'; // This variable source is very important for the QuizScreen to differentiate between 'normal Mcqs' and 'mock Mcqs' and 'pastPaper Mcqs' fetching function from controller

                                          Get.to(()=> QuizScreen(chapterClicked: pastPaperClicked, screenTitle: pastPaperItemName, source: source,));
...
class PastPapersShimmerItem extends StatelessWidget {

   const PastPapersShimmerItem({
    super.key,
  });
